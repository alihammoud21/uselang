...
flameshot gui --raw | uselang lens --stdin --lang fr` },
      ...
      {
        id: 'local-ai-setup',
        title: 'Local AI setup with LM Studio',
        desc: 'Run UseLang 100% offline using a free AI model on your own hardware.',
        content: [
          { type: 'p', text: 'UseLang can run entirely on your computer with no internet connection, using a local AI model served by LM Studio. Your voice and text never leave your machine.' },
          { type: 'h2', text: 'Requirements' },
          { type: 'list', items: [
            '8 GB RAM minimum (16 GB recommended for smooth performance)',
            '5 GB free disk space for the model',
            'macOS 13+, Windows 10+, or any modern Linux distro',
            'LM Studio 0.3.x or later (free download at lmstudio.ai)',
          ]},
          { type: 'h2', text: 'Setup' },
          { type: 'steps', items: [
            'Download and install LM Studio from lmstudio.ai.',
            'In LM Studio, go to the Search tab and type "gemma-3-4b-it".',
            'Click Download next to the Q4_K_M quantization (best balance of speed and quality).',
            'Once downloaded, go to the Local Server tab.',
            'Select the gemma-3-4b-it model and click Start Server. It will listen on port 1234.',
            'In UseLang (Settings → AI Provider), toggle "Use local AI" and confirm the server URL is http://localhost:1234.',
          ]},
          { type: 'h2', text: 'Verifying the connection' },
          { type: 'p', text: 'A green dot appears next to "Local AI" in UseLang\'s status bar when it can reach LM Studio. If you see a red dot, check that LM Studio\'s server is running and that no firewall is blocking port 1234.' },
          { type: 'tip', text: 'For Apple Silicon Macs, LM Studio uses the Neural Engine — gemma-3-4b runs at 40–80 tokens/second, fast enough for real-time conversation. On Intel or older AMD hardware, expect 8–20 tokens/second, which is still usable but slightly slower.' },
        ],
      },
    ],
  },
    ...
    articles: [
      {
        ...
        content: [
          { type: 'p', text: 'UseLang\'s offline mode lets you practice without any internet connection. The AI runs entirely on your device via LM Studio.' },
      ...
      },
      {
        ...
        content: [
          ...
          { type: 'list', items: [
            'Llama-3.2-3B-Instruct — slightly faster on CPU, slightly less fluent',
            'Phi-4-mini — great for low-RAM devices (under 4 GB)',
            'Mistral-7B-Instruct — higher quality, needs 12 GB+ RAM',
            'Any OpenAI-compatible API model — configure a custom endpoint in Settings',
          ]},
          { type: 'tip', text: 'On Apple Silicon (M1/M2/M3/M4), any model up to 13B runs at real-time speed thanks to the Neural Engine. On Windows with an NVIDIA GPU, set LM Studio to use CUDA for 3–5x speed improvements.' },
        ],
      },
    ],
  },
...
arecord -d 3 test.wav && aplay test.wav` },
      ...
      {
        id: 'lm-studio-not-connecting',
        title: "LM Studio won't connect",
        desc: 'Diagnose and fix connection issues with the local AI server.',
        content: [
          { type: 'h2', text: 'Checklist' },
          { type: 'list', items: [
            'LM Studio is open and a model is loaded.',
            'The Local Server tab shows "Server running" on port 1234.',
            'UseLang Settings → AI Provider shows http://localhost:1234.',
            'No VPN or firewall is blocking localhost traffic.',
          ]},
          { type: 'h2', text: 'Test the connection manually' },
          { type: 'code', lang: 'bash', text: `curl http://localhost:1234/v1/models` },
          { type: 'p', text: 'If this returns a JSON list of models, LM Studio is working. The problem is in UseLang\'s settings. If it fails, LM Studio\'s server isn\'t running.' },
          { type: 'h2', text: 'Common fixes' },
          { type: 'list', items: [
            'Restart LM Studio and reload the model before starting the server.',
            'Check that the port in LM Studio and UseLang match (default 1234).',
            'On macOS, check System Settings → Network → Firewall isn\'t blocking LM Studio.',
            'On Windows, check Windows Defender Firewall → Allow an app → add LM Studio.',
          ]},
        ],
      },
      {
      ...
      },
      {
        ...
        content: [
          { type: 'h2', text: 'Cloud AI sessions (online mode)' },
          { type: 'p', text: 'Sessions time out after 5 minutes of silence. Long pauses will cause the session to end. This is intentional to manage API costs for free tier users. Pro users have a 15-minute timeout.' },
          { type: 'h2', text: 'Local AI sessions' },
          { type: 'p', text: 'If a local AI session freezes, the model may have run out of memory. Try: closing other applications to free RAM, switching to a smaller/more quantized model, or restarting LM Studio.' },
          { type: 'h2', text: 'Connection lost mid-session' },
          { type: 'p', text: 'If you lose internet during a cloud session, UseLang tries to reconnect for 30 seconds. If reconnection fails and you have a local AI configured, it falls back to local mode automatically.' },
        ],
      },
    ],
  },
