...
| **Speak** | `app/(tabs)/train.tsx` | Voice tutor — Quick mode (type/speak → AI responds) and Phrase mode (drill pronunciation) |
...
lang/
├── apps/
│   ├── uselang-app/          ◄── React Native (Expo) mobile app
│   │   ├── app/              ◄── Screens (Expo Router file-based routing)
│   │   │   ├── (tabs)/       ◄── Bottom tab screens
│   │   │   └── ...           ◄── Modal & stack screens
│   │   ├── src/
│   │   │   ├── lib/          ◄── Core logic (AI, TTS, STT, stores)
│   │   │   ├── components/   ◄── Shared UI components
│   │   │   ├── hooks/        ◄── React hooks
│   │   │   └── data/         ◄── Lesson curricula (zh, es, fr)
│   │   ├── assets/           ◄── Fonts, models
...
└── firebase/                 ◄── Firestore rules (optional)
```
...
| **3D Globe** | `react-native-gl-model-view` + custom SVG |
...
| Asset | Size | Status |
|-------|------|--------|
| App Icon | 1024×1024 PNG (no alpha) | ⚠️ **You need to add this** — place at `apps/uselang-app/assets/icon.png` and reference in `app.json` |
| Splash Screen | 1284×2778 PNG | ⚠️ **You need to add this** — place at `apps/uselang-app/assets/splash.png` |
...
|-------------|--------|-------|
| AI calls per turn | 4 sequential (translate → phonetic → breakdown → pinyin) | **1 combined** (translate+phonetic), breakdown runs in background |
| Deepgram TTS timeout | 8s + retry (16s worst case) | **4s, no retry** → instant native fallback |
| TTS replay | Re-fetches from Deepgram each time | **In-memory LRU cache** (10 entries) |
| Silence detection | 2.0s after speech stops | **1.4s** (snappier, still safe) |
| Gemma GPU warmup | On conversation start | **On Train tab mount** (1–2s earlier) |
| Connectivity probe | Google (blocked in China) | **Apple + Cloudflare** (works globally) |
| Deepgram safety timeout | Fixed 30s | **Proportional** to text length (5–15s) |

## Notes

- The server (`server/`, `netlify/`) is **optional**. The mobile app works fully standalone.
- Firebase/Firestore is referenced in old web code only. The React Native app uses AsyncStorage exclusively.
- The `apps/web/` directory contains the marketing website, not the mobile app.
- The `apps/ios/` directory contains a native Swift iOS shell with offline lesson fallback (separate from the Expo app).
